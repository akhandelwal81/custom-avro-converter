/**
  * An [[OutputStream]] that writes the schema along with the messages.
  *
  ...
  * case the [[BinaryInputStream]] can be used.
  *
...
case class DataOutputStream[T](schema: Schema,
                                   out: OutputStream,
                                   codec: CodecFactory)
                                  (using encoder: Encoder[T]) extends OutputStream[T] {

  val (writer, writeFn) = schema.getType match {
    case Schema.Type.INT | Schema.Type.LONG | Schema.Type.FLOAT | Schema.Type.DOUBLE | Schema.Type.STRING =>
